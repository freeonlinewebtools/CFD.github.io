"""End-to-end proof that the muxed MP4 is actually playable.
...
the browser's own VideoEncoder, muxes them with src/mp4.js, and then hands the
...
Run:                                 python tests/mp4-play.py [port]
...
async () => {
  ...
  const cfg = { codec: 'avc1.42003E', width: W, height: H, bitrate: 1_200_000,
  ...
  const blob = muxMP4({ chunks, description, width: W, height: H, fps: FPS });
...
async def main():
    ...
    print("\nPASS — Chrome demuxed, seeked and decoded the muxed MP4"
